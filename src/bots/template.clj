...
(defn run [& args]
  (let [; 37
        numberOfCells (Integer/parseInt (read-line))]
    (dotimes [i numberOfCells]
      (let [; index: 0 is the center cell, the next cells spiral outwards
            ; richness: 0 if the cell is unusable, 1-3 for usable cells
            ; neigh0: the index of the neighbouring cell for each direction
            [index richness neigh0 neigh1 neigh2 neigh3 neigh4 neigh5] (map #(Integer/parseInt %) (str/split (read-line) #" "))]))
    (while true
